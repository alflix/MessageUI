...
public class SingleLineView: UIView {
    ...
    /// 线高, 默认 1.0，可以通过 GGUI.LineView.lineWidth 全局修改
    @IBInspectable public var lineWidth: CGFloat = GGUI.LineView.lineWidth
    /// 线的颜色  默认 lightGray，可以通过 GGUI.LineView.color 全局修改
    @IBInspectable public var lineColor: UIColor = GGUI.LineView.color {
        didSet {
            // 这个方法会调用 draw 方法
            setNeedsDisplay()
        }
    ...
    override public func draw(_ rect: CGRect) {
        ...
        if height > width {
            startPoint = CGPoint(x: width -  adjustPixelOffset, y: 0)
            endPoint = CGPoint(x: width - adjustPixelOffset, y: height)
        } else {
            startPoint = CGPoint(x: 0, y: height - adjustPixelOffset)
            endPoint = CGPoint(x: width, y: height - adjustPixelOffset)
        }
