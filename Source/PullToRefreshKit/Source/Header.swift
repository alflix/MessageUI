...
open class DefaultRefreshHeader: UIView, RefreshableHeader {
    ...
    public func stateDidChanged(_ oldState: RefreshHeaderState, newState: RefreshHeaderState) {
        ...
        if oldState == RefreshHeaderState.pulling && newState == RefreshHeaderState.idle {
            textLabel.text = textDic[.pullToRefresh]
            guard self.imageView.transform == CGAffineTransform(rotationAngle: -CGFloat.pi+0.000001) else { return }
            UIView.animate(withDuration: 0.4, animations: {
...
open class RefreshHeaderContainer: UIView {
    ...
    func handleScrollOffSetChange(_ change: [NSKeyValueChangeKey: Any]?) {
        ...
        let insetHeight: CGFloat = delegate.heightForRefreshingState?() ?? delegate.heightForHeader()
        let fireHeight: CGFloat = self.delegate?.heightForFireRefreshing?() ?? delegate.heightForHeader()
        if state == .refreshing {
        ...
        let topShowOffsetY = -1.0 * attachedScrollView.autualContentInset.top
        if offSetY > topShowOffsetY { return }
        let normal2pullingOffsetY = topShowOffsetY - fireHeight
